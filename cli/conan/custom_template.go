...
[light_green]命令[reset]:{{range .VisibleCategories}}{{if .Name}}
   [light_magenta]{{.Name}}[reset]:{{range .VisibleCommands}}
     [green]{{join .Names ", "}}[reset]{{"\t"}}[light_gray]{{.Usage}}[reset]{{end}}{{else}}{{range .VisibleCommands}}
	 [green]{{join .Names ", "}}[reset]{{"\t"}}[light_gray]{{.Usage}}[reset]{{end}}{{end}}{{end}}{{end}}{{if .VisibleFlagCategories}}
...
[light_yellow][bold]{{ .Name }} commands. Type "{{ .Name }} <command> -h" for help[reset]
`)
